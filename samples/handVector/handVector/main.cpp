		...
		if(NUI_SKELETON_TRACKED == state)
		{
			...
			*/
			...
			printf("x: %f %d\n",
				...
				posX = (int)(skeletonFrame.SkeletonData[i].SkeletonPositions[NUI_SKELETON_POSITION_HAND_RIGHT].x
				* 1000.0));

			printf("y: %f %d\n",
				skeletonFrame.SkeletonData[i].SkeletonPositions[NUI_SKELETON_POSITION_HAND_LEFT].y,
				posY = (int)(skeletonFrame.SkeletonData[i].SkeletonPositions[NUI_SKELETON_POSITION_HAND_RIGHT].y
				* 1000.0));


			rate = (posX - old_posX) / sqrt(pow((double)(posX - old_posX), 2) + pow((double)(posY - old_posY), 2));
			if ((posY - old_posY) < 0)
				rate *= -1; 
			//rate = -0.5;
			printf("rate: %lf\n", rate);
