...
int CMainWindow::Run(HINSTANCE hInstance, int nCmdShow)
{
	...
	// 추가
	m_windowStateID = IDS_WINDOW_MAIN;

	// Display window
	ResizeWindow();
	ShowWindow(m_hWndMain, nCmdShow); // 윈도우 생성
...
HRESULT CMainWindow::CreateMainWindow(HINSTANCE hInstance)
{
	...
	// 추가
	/*
	m_hBtnLearn = CreateWindowExW(0, STATUSCLASSNAME, NULL, WS_OVERLAPPED | WS_CHILD | BS_PUSHBUTTON, 100, 100, 300, 200, NULL,
		0, hInstance, NULL);
	*/

	m_hBtnLearn = CreateWindowExW(0, (LPCWSTR) "buttonLearn", NULL, WS_CHILD | BS_PUSHBUTTON, 0, 0, 100, 50, m_hWndMain,
		0, hInstance, NULL);
	if (!m_hBtnLearn)
	{
		return E_FAIL;
...
{   
	...
	// Color stream Information을 wstring 타입으로 받아서 colorStreamInfoText에 저장
	ReleaseMutex(m_hColorResolutionMutex);
	// 스레드간에 동기화 때문에 ReleaseMutex하는 것으로 보이나 아직 확실히 모름
	...
	// colorStreamInfoText를 포함하여 colorStream을 hdcBuffer 화면에 뿌림
	ReleaseMutex(m_hColorBitmapMutex);

	// Store width of color bitmap to properly position depth bitmap
	// depth bitmap을 적절한 위치에 놓기 위해 color bitmap의 너비를 구하여 저장
	BITMAP bmColor;
	...
	// Depth stream Information을 wstring 타입으로 받아서 depthStreamInfoText에 저장
	ReleaseMutex(m_hDepthResolutionMutex);
	// 스레드간에 동기화 때문에 ReleaseMutex하는 것으로 보이나 아직 확실히 모름
	...
	RECT statusRect;
	// 화면 하단에 statusBar를 나오게 하기 위해 RECT로 statusRect 생성
	GetWindowRect(m_hWndStatus, &statusRect);
	...
	// CreateCompatibleBitmap이 정확히 뭐하는 함수인지는 모르겠지만
	// 메인화면에 windowWidth, statusRect 높이 만큼의 공간을 만드는 함수로 보임
	...
	*/
	...
	// 추가
	// 아래에서 case문 바로 아래 {} 괄호를 한 것은
	// error c2360을 해결하기 위해서이다
	// 자세한 건 다음을 참고
	// http://sadiles.blog.me/10072136230
	// http://msdn.microsoft.com/en-us/library/61af7cx3(VS.71).aspx

	switch (m_windowStateID)
	{
	case IDS_WINDOW_MAIN:
		{
			// Determine dimensions of window
	RECT windowRect;
	GetClientRect(m_hWndMain, &windowRect); 
	...
	// 버튼
	RECT btnRect;
	GetWindowRect(m_hBtnLearn, &btnRect);
	int btnRectWidth = btnRect.right - btnRect.left;
	int btnRectHeight = btnRect.bottom - btnRect.top;

	HDC hdcBtnLearnBuffer = CreateCompatibleDC(hdc);
	HBITMAP hBtnLearnBitmap = CreateCompatibleBitmap(hdc, btnRectWidth, btnRectHeight);
	HGDIOBJ hOldBtnLearnBitmap = SelectObject(hdcBtnLearnBuffer, hBtnLearnBitmap);
	
	SendMessage(m_hBtnLearn, WM_PRINT, (WPARAM) hdcBtnLearnBuffer, PRF_CHILDREN | PRF_CLIENT);
	//BitBlt(hdcBuffer, windowWidth / 4 * 3, windowHeight / 4 * 3, btnRectWidth, btnRectHeight, hdcBtnLearnBuffer, 0, 0, SRCCOPY);
	
	SelectObject(hdcBtnLearnBuffer, hOldBtnLearnBitmap);
	DeleteDC(hdcBtnLearnBuffer);
	DeleteObject(hBtnLearnBitmap);
